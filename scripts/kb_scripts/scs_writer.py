  ...
  def write_edges(self, buffer, edges_queue, elements):
      ...
      for e in process_queue:
        ...
        try:
          ...
          try:
            ...
            msg = "Edge type `{}` is not supported. Please add it into EdgeTypes or into UnsupportedEdgeTypes".format(edge_type)
            self.log_error(msg)
    ...
    if len(edges_queue) > 0:
      ...
      for e in edges_queue:
        edge_ids += "{} ".format(e["id"])
      self.log_error("Wasn't able to create edges: {}".format(edge_ids))
